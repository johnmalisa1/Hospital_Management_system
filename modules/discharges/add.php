...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("INSERT INTO discharges (admission_id, doctor_id, discharge_date, summary, notes) VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("iisss", $admission_id, $doctor_id, $discharge_date, $summary, $notes);
    ...
    <form method="POST">
        ...
        <select name="admission_id" required>
            ...
            <?php while ($row = $admissions->fetch_assoc()): ?>
                <option value="<?= $row['admission_id'] ?>"><?= $row['patient_name'] ?></option>
            <?php endwhile; ?>
        </select>
